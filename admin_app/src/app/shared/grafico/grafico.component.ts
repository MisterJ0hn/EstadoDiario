...
// Registro único de los módulos que usa la consola. Se importa desde
// `echarts/core` (no desde `echarts`) para que el bundle no arrastre mapas,
// gráficos 3D ni el resto del catálogo que acá no se usa.
//
// Es el mismo componente que la app de los estudios, con una diferencia: allá
// se registran barras y torta además de líneas. Acá el único gráfico es la
// evolución de clientes, así que registrar el resto serían kilobytes que nadie
// dibuja. Si esta consola llega a tener un gráfico de barras, hay que agregar
// `BarChart` a esta lista o no se pinta nada y la consola no da error.
echarts.use([
  LineChart,
  GridComponent,
  LegendComponent,
