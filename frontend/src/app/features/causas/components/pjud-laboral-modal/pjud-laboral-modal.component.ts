...
@Component({
  ...
  template: `
    ...
    <ng-template #enlacePdf let-url let-tipo="tipo" let-color="color">
                                  ...
                                  @if (esIngresoCausa(h)) {
                                    ...
                                    <span class="inline-flex items-center justify-center gap-2">
                                      @for (doc of h.documentos; track $index) {
                                        <ng-container *ngTemplateOutlet="enlacePdf; context: { $implicit: doc.url, tipo: ($index === 1 ? 'certificado' : doc.tipo), color: doc.color }" />
                                      }
                                    </span>
                                  } @else { <span>-</span> }
                                </td>
...
export class PjudLaboralModalComponent implements OnDestroy {
  ...
  private iniciarPolling(causaId: number): void {
    ...
    this.pollTimer = setInterval(() => {
      this.service.pjudLaboral(causaId, false).subscribe({
        next: (res) => {
          this.datos.set(res);
          this.estadoPjud.emit({ causaId, estado: res.estado });
