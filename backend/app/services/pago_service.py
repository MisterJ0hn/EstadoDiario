...
class PagoService:
    ...
    def _reactivar_si_corresponde(self, factura: Factura) -> bool:
        ...
        **Las tres condiciones son todas necesarias**:
        ...
        # La reactivación por pago es la única que ocurre sin que nadie la
        # ordene, así que tiene que quedar en el historial como cualquier otra:
        # de esa tabla sale la serie mensual del dashboard de la consola, y sin
        # esta línea el cliente seguiría dibujándose suspendido para siempre.
        self.db.add(
            ClienteEstadoHistorial.de(
                cliente.cliente_id,
                activo=True,
                motivo=ClienteEstadoHistorial.MOTIVO_PAGO,
                actor="pago con Webpay",
            )
        )
        self.db.add(cliente)
        logger.warning(
